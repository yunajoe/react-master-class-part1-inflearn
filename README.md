...
c) 지수 백오프
지수 백오프(Exponential Backoff)는 네트워크나 서버 오류 발생 시, 재시도 간격을
(지수 함수) 형태로 점진적으로 늘려 서버 부하를 줄이고 성공률을 높이는 재시도 로직입니다. 처음에 짧은 대기 시간으로 시작해 실패가 반복될수록 대기 시간을 배로 늘리며, 무작위성을 더한 '지터(Jitter)'를 함께 사용하여 요청이 몰리는 현상을 방지
